...
from .preprocess import build_preprocessor, row_missing_fraction
...
class ComboResult:
    ...
    r2_mean: float | None
    r2_std: float | None
    pearson_r_mean: float | None
    pearson_r_std: float | None

    train_kept_frac_mean: float | None
    test_incomplete_frac_mean: float | None


def evaluate_combo_on_splits(
    *,
    df: pd.DataFrame,
    splits: list[Split],
    ...
    for s in splits:
        ...
        train_kept = train_df[mf <= threshold].copy()

        if train_kept.empty:
            failed_reason = "TRAIN_EMPTY_AFTER_THRESHOLD"
            break

        train_kept_fracs.append(len(train_kept) / len(train_df))

        incomplete_mask = test_df[numeric_cols].isna().any(axis=1).to_numpy()
        test_incomplete_fracs.append(float(incomplete_mask.mean()))
        ...
        X_train = train_kept[numeric_cols + categorical_cols]
        y_train = pd.to_numeric(train_kept[target_col], errors="raise").to_numpy()
    ...
    if failed_reason is not None or not fold_metrics:
        return ComboResult(
            model=model_spec.name,
            threshold=float(threshold),
            use_missing_indicator=bool(use_missing_indicator),
            status="FAILED",
            failed_reason=failed_reason,
            rmse_mean=None,
            rmse_std=None,
            ...
            pearson_r_std=None,
            train_kept_frac_mean=(
                float(np.mean(train_kept_fracs)) if train_kept_fracs else None
            ),
            test_incomplete_frac_mean=(
                float(np.mean(test_incomplete_fracs)) if test_incomplete_fracs else None
            ),
        )

    rmse_mean, rmse_std = mean_std([m.rmse for m in fold_metrics])
    mae_mean, mae_std = mean_std([m.mae for m in fold_metrics])
    mape_mean, mape_std = mean_std([m.mape for m in fold_metrics])
    r2_mean, r2_std = mean_std([m.r2 for m in fold_metrics])
    pr_mean, pr_std = mean_std([m.pearson_r for m in fold_metrics])
    ...
    return ComboResult(
        ...
        r2_mean=r2_mean,
        r2_std=r2_std,
        pearson_r_mean=pr_mean,
        pearson_r_std=pr_std,
        train_kept_frac_mean=(
            float(np.mean(train_kept_fracs)) if train_kept_fracs else None
        ),
        test_incomplete_frac_mean=(
