...
from ml.common.mlflow import init_mlflow
...
def main() -> int:
    ...
    _progress(
        f"Prepared {len(outer_splits)} OUTER folds: GroupKFold({GROUP_COL}), n_splits={OUTER_N_SPLITS}"
    )

    with mlflow.start_run(run_name=PARENT_RUN_NAME):
        mlflow.set_tags(
            {"design": "nested_cv", "model": "svr", "cv": "cv1_groupkfold_outer_inner"}
        )
        mlflow.log_params(
            {
            ...
            with mlflow.start_run(
                ...
                def inner_objective(trial: optuna.Trial) -> float:
                    ...
                    for inner in inner_splits:
                        ...
                        pipe = Pipeline([("pre", preprocessor), ("est", SVR(**params))])
                        pipe.fit(X_tr, y_tr)
                ...
                if gamma_mode in ("scale", "auto"):
                    final_params["gamma"] = gamma_mode
                else:
                    final_params["gamma"] = float(best_params["gamma"])

                final_pipe = Pipeline(
                    [("pre", preprocessor), ("est", SVR(**final_params))]
                )
